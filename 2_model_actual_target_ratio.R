...
arr.text = mutate(arr, x=0.02, label=paste(c('50%','90%'),'of studies')) # at far left
...
hplot = ggplot(data=studies, aes(x=ratio+0.001))+
  ...
  #scale_y_continuous()+ # set minimum as negative for labels; avoid tick marks below axis
  ...
  geom_text(data=arr.text, aes(x=x, y=y, label=label), hjust=0, col='darkseagreen4', size=3.5)+
...
hplot
jpeg('figures/histogram_sample_size_ratio.jpg', width=5.5, height = 4, units='in', res=300, quality=100)
print(hplot)
dev.off()
# for plos
tiff('figures/Fig1.tif', width=5, height=4, units='in', res=300, family='Times New Roman', compression = 'lzw') # PLOS
print(hplot)
invisible(dev.off())
...
# decided not to use plot, just table (see 3_regression_tables_latex.R)
#jpeg('figures/anzctr_sample_size_ratio.jpg', width=6.5, height=9.5, units='in', res=600, family='Times New Roman')
#tiff('figures/Fig3.tif', width=5, height=3, units='in', res=600, family='Times New Roman', compression = 'lzw') # PLOS
#print(to_plot)
#invisible(dev.off())
