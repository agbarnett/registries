# 3_model_actual_target_ratio_clintrials.R
...
labels1 = data.frame(x=1, y=58000, label='Above target') # y by trial and error
labels2 = data.frame(x=1, y=58000, label='Below target')
#
hplot = ggplot(data=studies, aes(x=ratio+0.001))+
  ...
  ylab('')+ # covered by ANZCTR
  ggtitle('clinicaltrials.gov')+
  theme_bw()+
  theme(legend.position = 'none',
        panel.grid.minor = element_blank())
save(hplot, file='figures/ratio_plot_clintrials.RData')
...
variable,colinear_with
allocationN/A,assignmentSingle Group
')

#
all_ests = variables_excluded = xval = NULL
standard_models = list()
cutoff = '1se' # just 1se
  set.seed(30991199) # for x-validation
  
  ## make design matrix
  vars_to_include = c('samplesize_target','date','gender','age_limit_max','age_limit_min','volunteers',
                      ...
                      'assignment','allocation','adaptive_trial','status')
  ...
  if(nrow(colinear) > 0){
    drop = colnames(X) %in% colinear$variable  # because of high VIF; NA phase is perfectly correlated with Observational study
    X = X[, !drop]
  }
...
## store estimates
save(all_ests, xval, standard_models, variables_excluded, file='results/sample_size_ratio_clintrials.RData')
