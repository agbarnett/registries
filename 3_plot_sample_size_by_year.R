...
clintrials = mutate(studies, year=as.numeric(format(submitted, '%Y'))) %>% # use date submitted
  filter(year < 2021, # remove few from 2021
         phase == "Phase 3", # phase 3 only (sensitivity analysis)
         lead_sponsor_class=="Industry")  # funded
...
## plot just clintrials
select = filter(clintrials_stats, year >= 2005) # remove initial years with fewer observations
cplot = ggplot(data=select, aes(x=year, y=tmean, ymin=tlower, ymax=tupper, fill=sample_size_type, col=sample_size_type))+
  geom_ribbon(alpha=0.5, color=NA)+ # colour NA to remove lines on outside
  geom_line(size=1.05)+
  scale_y_continuous(limits=c(0,NA))+ # start at zero
  scale_color_manual('Sample size', values=colours)+
  ...
  theme(legend.position = c(0.99,0.99),# top-right corner
        ...
        panel.grid.minor = element_blank()) 
cplot
jpeg('figures/sample_size_time_funded_phase3.jpg', width=5, height=4, units='in', res=300, family='Times New Roman') # 
print(cplot)
invisible(dev.off())
